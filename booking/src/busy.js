...
export function parseBusyIcs(text, windowStart, windowEnd) {
 ...
 * fetch has ever succeeded — callers must treat that as "cannot book now".
 */
export function createBusySource({ url, ttlSeconds, horizonDays, loader = defaultLoader, now = () => new Date() }) {
  ...
  return {
    async getBusyIntervals({ maxAgeSeconds = ttlSeconds } = {}) {
      const ageSeconds = cache ? (now().getTime() - cache.fetchedAt.getTime()) / 1000 : Infinity;
    ...
    start() {
      refresh().catch((err) => console.error('initial busy ICS fetch failed:', err.message));
      timer = setInterval(
        () => refresh().catch((err) => console.error('busy ICS poll failed:', err.message)),
        ttlSeconds * 1000,
      );
      timer.unref();
    },
